...
/// columns it does not name are never decoded. Restrict it up front —
...
/// `partitionValues`, not in the file) — the same pre-existing limitation as
/// the connector's `ListingTable` path.
...
mod tests {
    ...
    /// widens is cast (`Int32` → `Int64`), and a column the file lacks comes
