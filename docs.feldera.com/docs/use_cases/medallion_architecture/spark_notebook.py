...
def run_pipeline():
    ...
    enriched_clickstream = spark.sql("""
        SELECT
            ...
            ce.event_timestamp,
            c.customer_tier, c.signup_date,
            p.product_name,
            p.category AS product_category,
            p.brand AS product_brand,
            p.list_price AS product_list_price
        FROM bronze_clickstream_events ce
        LEFT JOIN silver_customers c ON ce.user_id = c.customer_id
        LEFT JOIN bronze_products p ON ce.product_id = p.product_id
    ...
    materialize(
        spark.sql("""
        WITH stream_max AS (
            SELECT MAX(event_timestamp) AS max_event_ts FROM silver_enriched_clickstream
        ),
        recent_demand AS (
            -- Browsing demand over a trailing 1-day window relative to the latest
            -- event in the stream (not wall-clock NOW(), which is empty on a
            -- historical snapshot). The window slides as events arrive, so products
            -- enter AND leave the alert in real time. Grouped by product_id only;
            -- attributes collapsed via MAX (null-safe, one row per product).
            SELECT
                ce.product_id,
                MAX(ce.product_name) AS product_name,
                MAX(ce.product_category) AS product_category,
                MAX(ce.product_brand) AS product_brand,
            ...
            CROSS JOIN stream_max m
            WHERE ce.product_id IS NOT NULL
                AND ce.event_timestamp > m.max_event_ts - INTERVAL 1 DAY
        ...
        SELECT
            d.product_id, d.product_name, d.product_category, d.product_brand,
...
# CDC tables with Debezium JSON format
CDC_TABLES = {
...
def read_cdc_files(table_short_name, up_to_hour=None, target_schema=None):
    """Read CDC NDJSON files, extract the 'after' records from the Debezium envelope.
    ...
    Args:
        ...
        target_schema: Schema from the snapshot Delta table. When provided, CDC columns
                       are cast to match (Debezium JSON serializes decimals as strings,
                       timestamps as ISO strings, etc.).

    Returns:
        DataFrame with the 'after' records (same schema as snapshot) plus an 'op' column.
    """
    ...
    # Debezium wraps each change in a "payload" envelope:
    #   {"payload": {"before": ..., "after": {...}, "op": "c"}}
    # The new row state lives in payload.after (null for deletes).
    if "payload" not in raw.columns:
        return None

    # Keep inserts/updates (after is populated); drop deletes (after is null).
    # Expand the 'after' struct into columns, keep 'op' for debugging.
    records = raw.filter(F.col("payload.after").isNotNull())
    after_cols = records.select("payload.after.*", F.col("payload.op").alias("op"))

    # Cast columns to match snapshot schema (Debezium JSON uses strings for decimals/timestamps)
    if target_schema:
...
def run_pipeline_with_cdc(up_to_hour=None):
    ...
    enriched_clickstream = spark.sql("""
        SELECT ce.event_id, ce.user_id, ce.session_id, ce.event_type, ce.page_url,
            ce.product_id, ce.device_type, ce.geo_country, ce.geo_region,
            ce.event_timestamp, c.customer_tier, c.signup_date, p.product_name,
...
# CDC comparison: process progressively more CDC data, full-refresh each time
CDC_CHECKPOINTS = [
    ("+ 1 hour", "2025-11-30T00"),
    ("+ 1 day", "2025-11-30T23"),
    ("+ 3 days", "2025-12-02T23"),
    ("+ 7 days", "2025-12-06T23"),
]

cdc_results = []
for label, hour in CDC_CHECKPOINTS:
    print(f"\n{'=' * 70}")
    print(f"  CDC CHECKPOINT: {label}  (through {hour})")
    print(f"{'=' * 70}\n")
    result = run_pipeline_with_cdc(up_to_hour=hour)
    cdc_results.append((label, hour, result))
    print(f"\n  Total: {result['total_time']:.1f}s\n")

# Print comparison table
print(f"\n{'=' * 70}")
print("  BATCH COST WITH CDC — Spark Full-Refresh Pipeline")
print(f"{'=' * 70}\n")
header = f"  {'CDC Window':<12} {'Orders':>9} {'Items':>9} {'Clicks':>9} {'Inv Evts':>9} {'Time':>8}"
print(header)
print("  " + "-" * (len(header) - 2))
for label, hour, r in cdc_results:
    rc = r["row_counts"]
    print(
        f"  {label:<12} {rc['orders']:>9,} {rc['order_items']:>9,}"
...
Every row above re-scanned the ENTIRE dataset (snapshot + all CDC to date)
and recomputed all 13 silver/gold views from scratch. The time is roughly
constant regardless of whether 1 hour or 7 days of CDC data were added.

With Feldera, each hourly CDC push
processes in milliseconds — cost proportional to CHANGE SIZE, not total size.

Run push_changes.py to see the Feldera side of this comparison:
...
print("""
WHAT FELDERA DOES DIFFERENTLY
==============================

The table above shows Spark re-scanning the full dataset at each checkpoint.
This is fundamental to batch. Cost is proportional to total dataset size.
If the size of the dataset were doubled, runtime would also roughly double

Feldera processed everything incrementally:

  - When 50 orders transition from 'pending' to 'shipped', only those 50 rows
    propagate through all 13 views — in under a second.
  - The per-commit processing latency is visible in the Grafana dashboard.
  - Cost is proportional to CHANGE SIZE, not total dataset size.

At 0.01x scale, the difference is milliseconds vs seconds. At 1.0x (~1B rows),
it's hours per batch run vs sub-second per commit.
""")
