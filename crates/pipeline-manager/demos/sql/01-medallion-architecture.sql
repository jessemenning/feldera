...
-- Clickstream events enriched with customer profile and product context.
-- Restricts to known interaction event types; LEFT JOINs so events whose
-- product or customer is unknown are still retained.
-- ----------------------------------------------------------------------------
...
recent_demand AS (
    -- Grouped by product_id only; descriptive attributes collapsed via MAX
    -- (null-safe, guarantees one row per product even if attributes vary/are null).
    SELECT
        ce.product_id,
