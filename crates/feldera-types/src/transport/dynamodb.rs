...
pub struct DynamoDBWriterConfig {
    ...
    /// Maximum retries for failed DynamoDB write chunks.
...
mod tests {
    ...
    fn table_name_validation() {
        ...
        // too short
        assert!(DynamoDBWriterConfig { table: "ab".to_string(), ..base.clone() }.validate().is_err());
        // minimum length
        assert!(DynamoDBWriterConfig { table: "abc".to_string(), ..base.clone() }.validate().is_ok());
        // too long
        assert!(DynamoDBWriterConfig { table: "a".repeat(256), ..base.clone() }.validate().is_err());
        // maximum length
        assert!(DynamoDBWriterConfig { table: "a".repeat(255), ..base.clone() }.validate().is_ok());
        // invalid characters
        assert!(DynamoDBWriterConfig { table: "my/table".to_string(), ..base.clone() }.validate().is_err());
        assert!(DynamoDBWriterConfig { table: "my table".to_string(), ..base.clone() }.validate().is_err());
        // valid characters
        assert!(DynamoDBWriterConfig { table: "my.table-name_1".to_string(), ..base.clone() }.validate().is_ok());
    }
}
